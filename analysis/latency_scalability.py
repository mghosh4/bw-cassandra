...
data_base_path = '../../data/latency_scalability'
...
def plot_throughput_vs_latency():
    ...
    # plt.figure()
    # plt.xlabel('Number of Cassandra nodes')
    # plt.ylabel('Average read latency (ms)')
    # plt.xlim(0, 16)
    # plt.ylim(0, 5)
    #
    # filtered_df = df[df['profile'] == 'emulab-ramdisk']
    # plt.scatter(x=filtered_df['num_cassandra_nodes'], y=filtered_df['read_average_latency'], label='emulab', marker=markers[0], color=colors[0])
    #
    # filtered_df = df[df['profile'] == 'bw']
    # plt.scatter(x=filtered_df['num_cassandra_nodes'], y=filtered_df['read_average_latency'], label='bw', marker=markers[1], color=colors[1])
    #
    # plt.legend(loc='best')
    # plt.savefig('%s/processed/%s/num-nodes-vs-read-latency-scatter.png' % (data_base_path, output_dir_name))
    #
    #
    #
    # plt.figure()
    ...
    #
    # filtered_df = df[df['profile'] == 'emulab-ramdisk']
    # plt.scatter(x=filtered_df['num_cassandra_nodes'], y=filtered_df['overall_throughput'], label='emulab', marker=markers[0], color=colors[0])
    #
    # filtered_df = df[df['profile'] == 'bw']
    # plt.scatter(x=filtered_df['num_cassandra_nodes'], y=filtered_df['overall_throughput'], label='bw', marker=markers[1], color=colors[1])
    #
    # plt.legend(loc='best')
    # plt.savefig('%s/processed/%s/num-nodes-vs-throughput-scatter.png' % (data_base_path, output_dir_name))
    ...
    # http://stackoverflow.com/questions/5938459/combining-plt-plotx-y-with-plt-boxplot
    # http://stackoverflow.com/questions/16090241/pandas-dataframe-as-input-for-matplotlib-pyplot-boxplot
    filtered_df = df[df['profile'] == 'emulab-ramdisk']
    grouped = filtered_df.groupby('num_cassandra_nodes')
    ...
    num_cassandra_nodes_list = flattened['num_cassandra_nodes'].values.tolist()

    box_data = []
    for i, num_cassandra_node in enumerate(num_cassandra_nodes_list):
        cur_df = filtered_df[filtered_df['num_cassandra_nodes'] == num_cassandra_node]
        box_data.append(cur_df['overall_throughput'].values.tolist())
    print box_data

    plt.boxplot(box_data, positions=num_cassandra_nodes_list)
    plt.xticks(locs, map(lambda x: int(x), locs))
    ...
    plt.plot(flattened['num_cassandra_nodes'].values.tolist(), flattened['mean'].values.tolist(), 'r-', label='bw')
    ...
    plt.savefig('%s/processed/%s/num-nodes-vs-throughput-box.png' % (data_base_path, output_dir_name))
    ...
    for i, num_cassandra_node in enumerate(num_cassandra_nodes_list):
        cur_df = filtered_df[filtered_df['num_cassandra_nodes'] == num_cassandra_node]
        box_data.append(cur_df['read_average_latency'].values.tolist())
    print box_data
    ...
    plt.savefig('%s/processed/%s/num-nodes-vs-latency-box.png' % (data_base_path, output_dir_name))
